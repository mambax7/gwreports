 ...
 *     limited mode version cannot add or modify autocomplete sql
 *
...
$GLOBALS['xoopsOption']['template_main'] = 'gwreports_reportedit.tpl';
...
$parameter_id = 0;
if (isset($_GET['pid'])) {
    $parameter_id = (int)$_GET['pid'];
}
if (isset($_POST['pid'])) {
    $parameter_id = (int)$_POST['pid'];
}
...
if ($report_id == 0) {
    redirect_header('newreport.php', 3, $err_message);
}
...
if ($op === 'update') {
    ...
    if (!$dberr) {
        ...
        if ($xoopsDB->errno() == 1062) {
            ...
            $err_message = _MD_GWREPORTS_PARAMETER_UPD_ERR . ' ' . $dbmsg;
        }
    }
}
...
$form->addElement(new XoopsFormLabel($caption, nl2br(htmlspecialchars($parameter_sqlchoice, ENT_QUOTES)), 'parameter_sqlchoice'), false);
