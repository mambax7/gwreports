<?php
/**
 * report_print.php - display report in web page for printing
 *
...
foreach ($parameters as $v) {
    ...
    if ($parm_required) {
        if (!(isset($_GET[$parm_shortname]) || isset($_POST[$parm_name]))) {
            $all_required_supplied = false;
        }
        if ($parm_value == '') {
            $all_required_supplied = false;
        }
...
if ($op === 'run') {
    ...
    foreach ($sections as $s) {
        ...
        if ($dbmsg != '') {
            $body .= '<br /><em>' . sprintf(_MD_GWREPORTS_RUNTIME_SQL_ERROR, $dbmsg) . '</em><br />';
        }
    } // foreach ($sections as $s)
}
