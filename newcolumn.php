...
$GLOBALS['xoopsOption']['template_main'] = 'gwreports_columnedit.tpl';
...
if (isset($_POST['sid'])) {
    $section_id = (int)$_POST['sid'];
}

if (isset($_GET['cname'])) {
    $column_name = cleaner($_GET['cname']);
}
if (isset($_POST['cname'])) {
    $column_name = cleaner($_POST['cname']);
}
...
if (isset($section['section_name'])) {
    ...
    redirect_header('index.php', 3, _MD_GWREPORTS_SECTION_NOTFOUND);
}
...
if (isset($_POST['column_name'])) {
    $column_name = cleaner($_POST['column_name']);
}
if (isset($_POST['column_title'])) {
    $column_title = cleaner($_POST['column_title']);
}
if (isset($_POST['column_hide'])) {
    $column_hide = cleaneryn($_POST['column_hide']);
}
if (isset($_POST['column_sum'])) {
    $column_sum = cleaneryn($_POST['column_sum']);
}
if (isset($_POST['column_break'])) {
    $column_break = cleaneryn($_POST['column_break']);
}
if (isset($_POST['column_outline'])) {
    $column_outline = cleaneryn($_POST['column_outline']);
}
if (isset($_POST['column_apply_nl2br'])) {
    $column_apply_nl2br = cleaneryn($_POST['column_apply_nl2br']);
}
if (isset($_POST['column_is_unixtime'])) {
    $column_is_unixtime = cleaneryn($_POST['column_is_unixtime']);
}
if (isset($_POST['column_format'])) {
    $column_format = cleaner($_POST['column_format']);
}
if (isset($_POST['column_style'])) {
    $column_style = cleaner($_POST['column_style']);
}
if (isset($_POST['column_extended_format'])) {
    $column_extended_format = cleaner($_POST['column_extended_format'], true);
}
...
if ($op === 'add') {
    ...
    if (!$dberr) {
        ...
        if ($xoopsDB->errno() == 1062) {
            $err_message = _MD_GWREPORTS_COLUMN_DUPLICATE;
        } else {
            $err_message = _MD_GWREPORTS_COLUMN_ADD_ERR . ' ' . $dbmsg;
        }
    }
}
